...
# First we start by selecting the protocol and entering the IP address of the server
        ...
        if mode == "download":
            ...
            if serverMsg.decode() == "Request accepted":  # the server accepted the request and will start sending
                ...
                while True:  # loop used to receive the data
                    ...
                    t2 = time.time()  # update t2 to the time after receiving a new chunk of data
                    size = size + len(data)  # add to size the size of the new data received (in bytes)
            ...
            while data:  # while there is still data to be read in the file
                ...
                size = size + len(data)  # add to size the size of the last chunk of data sent
